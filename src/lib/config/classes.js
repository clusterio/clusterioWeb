...
class Config extends events.EventEmitter {
	constructor() {
		...
		if (!this.constructor._finalized) {
			// TODO - Figure out why this throws on new config.InstanceConfig(); when creating instance
			console.error(new Error(`Cannot instantiate incomplete Config class ${this.constructor.name}`));
		}
...
class ConfigGroup {
	...
	constructor() {
		if (!this.constructor._finalized) {
			console.error(new Error(`Cannot instantiate incomplete ConfigGroup class ${this.constructor.name}`));
		}
	...
	set(name, value) {
		...
		this._fields.set(name, value);
		if (this._config && !deepEqual(value, prev)) {
	...
	setProp(name, prop, value) {
		...
		if (this._config && !deepEqual(updated, prev)) {
			this._config.emit("fieldChanged", this, name, prev);
		}
	}
	...
	update(serializedGroup, notify) {
		...
		for (let [name, value] of Object.entries(serializedGroup.fields)) {
			...
			let prev = this._fields.get(name);
			this._fields.set(name, value);
			if (notify && this._config && !deepEqual(value, prev)) {
				this._config.emit("fieldChanged", this, name, prev);
			}
		}
	}
